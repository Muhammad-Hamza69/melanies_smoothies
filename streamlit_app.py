...
if ingredients_list:
    ...
    for fruit_chosen in ingredients_list:
        # 1. Yahan fruit_chosen[0] likhna zaroori hai kyunke fruit_chosen ek 'Row' object hai
        fruit_name = fruit_chosen[0] 
        ingredients_string += fruit_name + ' '
        
        st.subheader(fruit_name + ' Nutrition Information')
        
        # 2. API call mein ab sahi fruit_name jayega
        ...
        if smoothiefroot_response.status_code == 200:
            ...
            st.error(f"Could not find nutrition info for {fruit_name}")
