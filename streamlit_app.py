...
# # 1. Data fetch karein (FRUIT_NAME aur SEARCH_ON dono)
# my_dataframe = session.table("smoothies.public.fruit_options").select(col('FRUIT_NAME'), col('SEARCH_ON'))

# # 2. Snowpark Dataframe ko Pandas mein convert karein
# pd_df = my_dataframe.to_pandas()

# # 3. Multiselect mein Pandas ka 'FRUIT_NAME' column pass karein
# # Is se fruit_chosen hamesha ek clean string ('Apples') hogi, Row object nahi.
# ingredients_list = st.multiselect(
#     'Choose up to 5 ingredients: ',
#     pd_df['FRUIT_NAME'], 
#     max_selections = 5
...
#         # 4. Search value nikalna (Ab yeh crash nahi hoga)
#         search_on = pd_df.loc[pd_df['FRUIT_NAME'] == fruit_chosen, 'SEARCH_ON'].iloc[0]
#         st.write('The search value for ', fruit_chosen, ' is ', search_on, '.')
        
#         # 5. Heading mein asli naam dikhayein
#         st.subheader(fruit_chosen + ' Nutrition Information')
        
#         # 6. API call mein 'search_on' wala name use karein (e.g. 'Apple' instead of 'Apples')
#         # Is se 'Not Found' wala error khatam ho jayega
